...
void mbproduct(dt *A,dt *B,dt *C,int a,int b,int c,int r){
	dt *d_A = NULL;	//a*r  *c
	dt *d_B = NULL;	// b*r  *c
	dt *d_C = NULL; //a*b *c
	cudaMalloc((void**)&d_A,sizeof(dt)*a*r*c); 		
	cudaMalloc((void**)&d_B,sizeof(dt)*r*b*c);		
	cudaMalloc((void**)&d_C,sizeof(dt)*a*b*c);
	cudaMemcpy(d_A,A,sizeof(dt)*a*r*c,cudaMemcpyHostToDevice);
	cudaMemcpy(d_B,B,sizeof(dt)*r*b*c,cudaMemcpyHostToDevice);
	dt alpha = 1.0;
	dt beta = 0.0;
	cublasHandle_t handle;
	cublasCreate(&handle);
	...
	cudaMemcpy(C,d_C,sizeof(dt)*c*b*a,cudaMemcpyDeviceToHost);
	cublasDestroy(handle);
	cudaFree(d_A);
	cudaFree(d_B);
	cudaFree(d_C);
