...
void basematrixpro(dt *A,dt *res,int row){
	...
	dt *d_A;
	dt *d_res;
	cudaMalloc((void**)&d_A,sizeof(dt)*row*row);
	cudaMalloc((void**)&d_res,sizeof(dt)*row*row);
	dt alpha = 1.0;
	dt beta = 0.0;
	cudaMemcpy(d_A,A,sizeof(dt)*row*row,cudaMemcpyHostToDevice);
	...
	cudaMemcpy(res,d_res,sizeof(dt)*row*row,cudaMemcpyDeviceToHost);
	cudaFree(d_A);
	cudaFree(d_res);
